...
scene.background = new THREE.Color(0x000000); // Set to black or any other contrasting color
...
fullScreenMesh.renderOrder = 999; // Ensure it's rendered last
...
function animate() {
    ...
    for (let i = 0; i < positionAttribute.count; i++) {
        ...
        // Increased amplitude for larger wave pattern
        const waveX = 0.4 * Math.sin(x * 2 + Date.now() * 0.001 + mouseX * 5); // Increased amplitude
        const waveY = 0.2 * Math.sin(y * 3 + Date.now() * 0.0015 + mouseY * 5); // Increased amplitude

        positionAttribute.setZ(i, waveX + waveY);
    }
    positionAttribute.needsUpdate = true;

    //renderer.clearDepth(); // Clear the depth buffer before rendering the gradient plane
